...
const tank=(ref='tank'):Unit=>({ref,name:'MTNK',type:7,x:30,y:30,hp:400,maxHp:400,width:1,height:1,mobile:true,idle:true,harvester:false,mcv:false,yard:false,refinery:false,combat:true});
const observation=(own:Unit[]=[tank()]):Observation=>({tick:0,credits:10000,power:{total:200,drain:0,isLowPower:false},home:{x:25,y:25},starts:[{x:25,y:25},{x:70,y:70}],own,enemies:[],products:[],queues:[],buildSites:[]});
...
test('a vanished enemy becomes a remembered position, never a new object-target attack',()=>{
  ...
  assert.ok(policy.decide(o).some(i=>i.kind==='attack'&&i.target==='contact'));
  const later={...o,tick:450,enemies:[]};
  const intents=policy.decide(later);
  assert.ok(!intents.some(i=>i.kind==='attack'));
  assert.ok(intents.some(i=>i.kind==='attackMove'&&i.x===35&&i.y===35));
});
