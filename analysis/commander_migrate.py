...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--input',required=True);ap.add_argument('--out',required=True);ap.add_argument('--encoding',required=True,choices=['graph-plan-v1','graph-plan-v2','graph-plan-v3']);args=ap.parse_args()
    ...
    sha=export(model,out,{'method':'action-grammar-migration','git':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),'inputSha256':hashlib.sha256(source.read_bytes()).hexdigest(),'parentEncoding':artifact['encoding'],'sharedWeightsChanged':False,'parametersAdded':added,'policyChanged':artifact['encoding']!=args.encoding,'scope':'Action grammar or edit gates change the policy; collect fresh PPO data'})
    ...
    out.with_suffix('.golden.json').write_text(json.dumps(result)+'\n');print(json.dumps({'sha256':sha,'encoding':args.encoding,'sharedWeightsChanged':False,'parametersAdded':added,'policyChanged':artifact['encoding']!=args.encoding}))
