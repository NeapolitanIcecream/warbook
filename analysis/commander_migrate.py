...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--input',required=True);ap.add_argument('--out',required=True);ap.add_argument('--encoding',required=True,choices=['graph-plan-v1','graph-plan-v2','graph-plan-v3']);ap.add_argument('--temperature',type=float);args=ap.parse_args()
    torch.set_num_threads(1);source=Path(args.input);artifact=json.loads(source.read_text());model=load_model(artifact);before=sum(p.numel() for p in model.parameters());model.change_encoding(args.encoding)
    ...
    policy_changed=artifact['encoding']!=args.encoding or artifact.get('temperature',1.)!=model.temperature
    out=Path(args.out);out.parent.mkdir(parents=True,exist_ok=True)
    sha=export(model,out,{'method':'policy-reparameterization','git':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),'inputSha256':hashlib.sha256(source.read_bytes()).hexdigest(),'parentEncoding':artifact['encoding'],'parentTemperature':artifact.get('temperature',1.),'sharedWeightsChanged':False,'parametersAdded':added,'policyChanged':policy_changed,'scope':'Grammar, edit gates or temperature can change the policy; collect fresh PPO data'})
    ...
    out.with_suffix('.golden.json').write_text(json.dumps(result)+'\n');print(json.dumps({'sha256':sha,'encoding':args.encoding,'temperature':model.temperature,'sharedWeightsChanged':False,'parametersAdded':added,'policyChanged':policy_changed}))
