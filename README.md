...
**当前默认：0.1.12；0.1.11 保留为对照。** 本轮修正远处建筑错误覆盖近处交战，以及发现敌方基地后仍追逐过时探索坐标的问题。四图固定开发池新版 45/48 胜、同批 0.1.11 为 39/48；新版另两负、一次上限，旧版另八负、一次双方败北未决。起点未逐局配对，收益不外推到未见地图或真人对战。取舍与验证见 [自主迭代记录](docs/post-0111-cycle.md#0112-发布验收)。
...
旧版本对照入口为 **[0.1.11 AI](http://127.0.0.1:8642/challenge/)**。游戏页下方显示所选 AI 的版本。旧 0.1.7、0.1.8 阵地路线和 counter 继续保留为冻结研究对手。
...
只想看 AI 对战，可以打开 **[0.1.12 对 Supalosa 的完整回放](http://127.0.0.1:8642/watch)**：短图，11:04，WarbookRed 获胜。6:30 可看防守，7:09 发起反击、7:40 主力已离家，8:48 起敌建筑实际受损；仍损失一座电厂、三名步兵和一条军犬。回放右下角可输入时刻并「跳到并暂停」，向后跳会从开头重新推进。也保留 **[历史移动原型的失败记录](http://127.0.0.1:8642/game/#/replay/http%3A%2F%2F127.0.0.1%3A8642%2Fwarbook%2Freplay-check.rpl)**：60:00 达到运行上限，双方未败北，不计正常败局。可用底部按钮加速，或 Esc → 全屏展开画面。
