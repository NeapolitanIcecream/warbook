...
**当前玩家稳定版为 0.1.16，主线与压制共同更新。** 本轮补齐原生命令持续执行、矿车遇袭返矿、坦克随实际采矿区域转移屏卫。四图同池主线 **25/32 胜、全部正常结束**，0.1.15 对照 **19/32 胜、1 次上限**；池外另有一场低资金残局上限，保留在 [发布记录](docs/strategy-016.md)。学习研究已接入主力持续作战控制并完成6,944局比较；本轮更大权限尚未显示稳定收益，继续保留窄权限学习候选，玩家默认不变。见 [本轮结果](docs/operation-control-cycle.md)。
...
也可选择 **[0.1.15 对照](http://127.0.0.1:8642/challenge/)** 或 **[0.1.16 步兵压制](http://127.0.0.1:8642/specialist/)**。压制路线保留双路步兵和装甲协同，并共享矿车保护与矿区屏卫；城市/短图对同版主线为3/4胜、全部正常结束。两路线按完整包独立冻结。
...
当前 **[观看回放](http://127.0.0.1:8642/watch)** 为 **0.1.16 主线对 Supalosa** 的短图胜局，时长 **11:12**。双方玩家名带策略和版本；右下角可输入时刻并「跳到并暂停」，底部可加速。更强对手的完整比较和池外上限均保留在发布记录；原始回放仅存本地。

这是基础地面作战研发版。当前重点是建设、采矿、出兵、坦克对步兵作战和寻找迁移的基地；不宣称已经覆盖所有兵种、地图或真人竞技水平。

## 本地开发与复现

使用已验证的 Node.js 26.5.0（见 `.node-version`）和合法取得的 `ra2.mix`、`language.mix`、`multi.mix`。Node 24.0.0 的同步加载钩子与当前 tsx 组合在服务器上失败；不要仅依据最低 API 版本选择运行时。
...
`bastion` 是当前默认策略：独立军犬探索敌基地和未知可达区；步兵与车辆保护受威胁的建筑和矿车；主力按实际成组情况、可见/记忆敌军和可能来援判断出击，行军中重新评估。未知目标仍先组织六车探索；已知机会不再使用固定四车门槛。`factory-exit` 是保留的 0.1.7 路线，`combined` 是更早主线，`counter` 是实验对手。模式名称本身不冻结代码，跨版本比较必须使用冻结包。全部实验和固定试玩版本见 [执行进展](docs/progress.md)。

`bastion` 使用分别归属的主力、守备、预备、汇合、侦察与恢复任务；恢复中的单位不会锁住其余主力。`cohort-local` 是未晋升的主线编组研究模式。移动射击原型出现过三次上限，已从当前运行代码移除，冻结历史和失败记录见 [阵地与编组实验](docs/defense-cycle.md)。当前具有有限伤员轮换、军犬屏护和光棱发展；尚不等于成熟的人类 T 优或完整中后期能力。
...
- 0.1.16：三项基础行为及四图完整比较见 [本版记录](docs/strategy-016.md)；139项测试，五份选定回放1,034快照匹配，两路线发布同步13,975次指令零差异。
...
- 策略使用 `api-shroud-v1-pregame-map-prior`：仅当前己方状态、按本方 shroud 过滤的敌方接触、己方探索信息；建筑预检局限于已探索的基地周边。该工程条件尚不宣称与人类界面的隐形、伪装等观察完全等价。
