...
**当前本地版本为 0.1.14，阵地反击与步兵压制两条路线均已更新。** 本轮修复防守交战被远征撤退打断、后排坦克不参战、步兵不支援近处装甲，以及低资金时无法补侦察军犬的问题；保留分路布防、可达选位和来路碉堡。四图四对手同批完整局，新版 **26/32 胜**，0.1.12 **23/32 胜**，上限分别一次、两次；城市仍有退化切片，详见 [本轮记录](docs/strategy-014.md)。
...
也可选择 **[0.1.12 对照](http://127.0.0.1:8642/challenge/)** 或 **[0.1.14 步兵压制](http://127.0.0.1:8642/specialist/)**。压制路线让首波两组同时成军，并按完整小队补充；对冻结新版防守的短图/城市检查为 3/4 胜，旧压制版为 1/4 胜、一次上限。这是专长测试，不代表所有对手上都更强。两路线按完整包独立冻结，历史版本继续保留。
...
当前 **[观看回放](http://127.0.0.1:8642/watch)** 已按用户请求切到 **0.1.14 主线评估候选输给 Supalosa** 的城市局，时长 **16:00**：黄色为主线，红色为 Supalosa。这是原始失败记录，旧玩家名保持不变。原发布展示局（26:15 胜旧阵地）仍保存在本地，见 [发布记录](docs/strategy-014.md#展示与已知缺口)。回放右下角可输入时刻并「跳到并暂停」，向后跳会从开头重新推进；底部可加速。
