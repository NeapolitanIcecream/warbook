...
## 2026-09-19：赛前地图与可见中立目标

候选 0.1.15 的 `api-shroud-v1-pregame-map-prior` 增加赛前地图先验。`MapPrior.readPregame` 只在第 0 tick、任何模拟推进前运行，读取 `Tile.terrainType/z`、初始桥面 overlay 高度和中立建筑轮廓。后续只持有纯坐标图，没有引擎对象、玩家单位或原生 ID；不调用 `getAllUnits`、全图当前资源、`findPath` 或 `getReachabilityMap`。固定地图中立建筑与初始桥是允许的地图知识，战斗方实际出生分配不进入先验。桥梁后续损毁和新建筑不是静态图保证可通过的内容，最终路径仍由原生命令执行，已探索驻守目标另查实时通行性。

工程师目标来自 `getVisibleUnits("hostile", capturable && produceCashAmount > 0)`，该接口包含中立对象、排除己方和盟友；只有当下合法可见的目标可提交 Capture。源码/声明定位为 SDK `MapApi`、`GameApi.getVisibleUnits`、`Tile`、`OrderType.Capture`。运行上，第二轮城市图六局均至少拥有过一座 CAOILD；一次所有权变化可能对应多条待确认命令，反馈条数不能当成占领次数。

普通步兵/坦克接触消失时，若上一观察在三 tick 内、其保守三格移动邻域全部已探索，就将该旧位置记录为 `vacatedContacts`。不查询消失对象，不读取全局死亡事件，也不把它报告为确认击杀；矿车传送、基地车、特殊单位不适用。用于及时释放旧位置威胁，合法可见对象重新出现时仍正常更新。

这是信息条件的显式扩展。与 0.1.14 比较时报告为整包比较，不能称仅改变战略的消融。原生回放检查仍独立执行；初步第二轮城市 Supalosa 局 82/82 原单位快照及终止状态匹配，其他验证见 `docs/iteration-015.md`。
