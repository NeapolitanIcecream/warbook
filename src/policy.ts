...
export class Commander {
  ...
  decide(o: Observation): Intent[] {
    ...
    const names = allied
      ? { power:'GAPOWR', refinery:'GAREFN', barracks:'GAPILE', factory:'GAWEAP', tank:'MTNK', miner:'CMIN', infantry:'E1' }
      : { power:'NAPOWR', refinery:'NAREFN', barracks:'NAHAND', factory:'NAWEAP', tank:'HTNK', miner:'HARV', infantry:'E2' };
    ...
    const queue = (name: string | undefined) => {
      ...
      if (!q || q.size || q.status !== 0 || o.credits < Math.min(product.cost,250)) return;
      if (!intents.some(i=>i.kind==='queue' && i.product.queue===product.queue)) intents.push({kind:'queue',product});
    };
    let building: string | undefined;
    if (o.power.isLowPower || (count(names.power) && o.power.total-o.power.drain < 30)) building=names.power;
    ...
    else if (count(names.factory)<2 && o.credits>3500) building=names.factory;
    queue(building);
    ...
    const order = (units: Unit[], key: string, intent: Intent, repeat=150) => {
      const refs = units.filter(u=>{
        const prev=this.lastOrders.get(u.ref);
        return !prev || (prev.key!==key && o.tick-prev.tick>=30) || o.tick-prev.tick>=repeat;
      }).map(u=>u.ref);
    ...
    if (this.mode==='cohesive' && !threat && army.length<8 && o.tick<4500) {
      const scout=army.slice(0,1);
      order(scout,'scout:'+destination.x+':'+destination.y,{kind:'attackMove',refs:[],x:destination.x,y:destination.y},300);
      const rally={x:o.home.x+4,y:o.home.y+4};
      order(army.slice(1),'rally',{kind:'attackMove',refs:[],...rally},450);
    } else {
